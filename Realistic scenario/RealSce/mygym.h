...
public:
  ...
  virtual Ptr<OpenGymSpace> GetActionSpace(uint32_t agent_id) = 0; // done;
  virtual Ptr<OpenGymSpace> GetObservationSpace(uint32_t agent_id) = 0; // done;
  virtual Ptr<OpenGymDataContainer> GetObservation(uint32_t agent_id) = 0; // done;
  virtual float GetReward(uint32_t agent_id) = 0; // done;
  virtual bool GetDone(uint32_t agent_id) = 0; // done;
  virtual std::string GetInfo(uint32_t agent_id) = 0; // done;
  virtual bool ExecuteActions(uint32_t agent_id, Ptr<OpenGymDataContainer> action) = 0; // done;
  ///\}
  ...
  void calculate_rewards(uint32_t agent_id); // done;
...
private:
  ...
  uint8_t HO_possible[m_cellCount][m_cellCount]; // Added
  std::vector<uint32_t[2]> cio_list; // Added
  std::vector<uint32_t> m_cellFrequency;
  // std::vector<uint32_t> m_cellFrequencyVec; // Removed
  std::vector<std::vector<uint32_t>> m_UesNum; // Added
  ...
  // std::vector<float> rewards; // Removed
  std::vector<std::vector<float>> rewards; // Added
  //Added
  // std::vector<float, float> rewards;
  //
